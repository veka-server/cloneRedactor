...
/* POPUP */
	...
	ready(function()
	{
	    ...
	    cloneRedactor.argsToArray(document.getElementsByClassName('cloneRedactor')).forEach(function(element, index, array)
	    {
	      if(element.getAttribute("data-outils"))
	      {
	        var outils = element.getAttribute("data-outils").split(',');
	        cloneRedactor.init(element, outils );
	      }
	      else
	      {
	        cloneRedactor.init(element);
	      }
	    });
	});
